...
class Trainer(tf.Module):
    ...
    def train_step(self, x, y):
        ...
        for _ in range(self.steps_per_update):
            with tf.GradientTape() as wp_tape:
                ...
                loss_total = tf.reduce_mean(loss_pll) #+ self.coef_pessimism * tf.reduce_mean(loss_pessimism)
    ...
    def ema_step(self):
        # copy the current model's weight into the old model
        for ema_weight, weight in zip(self.model.trainable_variables, self.temp_model.trainable_variables):
            ema_weight.assign(ema_weight * 0.95 + weight * 0.05)
